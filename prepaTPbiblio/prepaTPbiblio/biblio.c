...
int Emprunter (T_Bibliotheque  *ptrB, T_livre *livre, const T_Emp *nom)
{
    int i;
    if (positionLivre(ptrB,livre->titre)!=-1)
    {
        for (i=0;i<ptrB -> nbLivres;i++)
        {
            if ((strcmp(livre->code,ptrB->etagere[i].code)==0) && (strcmp(livre->titre,ptrB->etagere[i].titre)==0)&&(strcmp(ptrB->etagere[i].emprunteur,"") == 0))
            {
